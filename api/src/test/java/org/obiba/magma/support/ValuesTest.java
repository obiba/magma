...
public class ValuesTest extends AbstractMagmaTest {
  ...
  public void test_asValues_returnsValuesOfProperType() {
    ...
    assertThat(Iterables.elementsEqual(values, ImmutableList.<Value> of(IntegerType.get().valueOf(1), IntegerType.get().valueOf(2))), is(true));
  }

  @Test
  public void test_asValues_handlesNull() {
    Iterable<Value> values = Values.asValues(IntegerType.get(), 1, null);
    assertThat(Iterables.elementsEqual(values, ImmutableList.<Value> of(IntegerType.get().valueOf(1), IntegerType.get().nullValue())), is(true));
  }
